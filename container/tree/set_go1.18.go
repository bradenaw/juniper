...
type Set[T any, O xsort.Ordering[T]] struct {
	t *tree[T, struct{}, O]
}
...
func NewSet[T any, O xsort.Ordering[T]]() Set[T, O] {
	return Set[T, O]{
...
func (s Set[T, O]) Iterate() iterator.Iterator[T] {
	return s.Cursor().Forward()
}

// Cursor returns a cursor into the set placed at the first item.
func (s Set[T, O]) Cursor() *SetCursor[T, O] {
	inner := s.t.Cursor()
	return &SetCursor[T, O]{
		inner: inner,
	}
}
...
type SetCursor[T any, O xsort.Ordering[T]] struct {
...
func (c *SetCursor[T, O]) SeekFirst() { c.inner.SeekFirst() }

// SeekLast moves the cursor to the last item in the set.
//
// SeekLast is O(log(n)).
func (c *SetCursor[T, O]) SeekLast() { c.inner.SeekLast() }

// SeekLastLess moves the cursor to the item in the set just before x.
//
// SeekLastLess is O(log(n)).
func (c *SetCursor[T, O]) SeekLastLess(x T) { c.inner.SeekLastLess(x) }
...
func (c *SetCursor[T, O]) SeekLastLessOrEqual(x T) { c.inner.SeekLastLessOrEqual(x) }
...
func (c *SetCursor[T, O]) SeekFirstGreaterOrEqual(x T) { c.inner.SeekFirstGreaterOrEqual(x) }

// SeekFirstGreater moves the cursor to the item in the set just after x.
//
// SeekFirstGreater is O(log(n)).
func (c *SetCursor[T, O]) SeekFirstGreater(x T) { c.inner.SeekFirstGreater(x) }
...
// case it's O(log(n)).
func (c *SetCursor[T, O]) Next() { c.inner.Next() }
...
func (c *SetCursor[T, O]) Forward() iterator.Iterator[T] {
...
// This iterator's Next method is amoritized O(1), unless the map changes in which case the
// following Next is O(log(n)) where n is the number of elements in the map.
func (c *SetCursor[T, O]) Backward() iterator.Iterator[T] {
